...
if use_center == 0 :
    f = open("./log_dir/"+dataset+"/"+str(split)+"/ts_resnet101/rgb/video_predict_multi.pickle",'rb')
    g = open("./log_dir/"+dataset+"/"+str(split)+"/ts_resnet101/flow/video_predict_multi.pickle",'rb')
    print('use ten crop')
    if fusion_ratio != 0:
        if nl == 0:
            fusion = open("./log_dir_with_two_stream_m2d/"+dataset+"/"+str(split)+"/ts_resnet101/fusion/fhn/"+fm+"/video_predict_multi.pickle",'rb')
            print('use local fusion')
        else:
            fusion = open("./log_dir_with_two_stream_m2d/"+dataset+"/"+str(split)+"/ts_resnet101/fusion/non_local/"+fm+"/video_predict_multi.pickle",'rb')
            print('use non-local fusion')
else:
    print('use center crop')
    f = open("./log_dir/"+dataset+"/"+str(split)+"/ts_resnet101/rgb/video_predict_center.pickle",'rb')
    g = open("./log_dir/"+dataset+"/"+str(split)+"/ts_resnet101/flow/video_predict_center.pickle",'rb')
    if fusion_ratio != 0:
        if nl == 0:
            fusion = open("./log_dir_with_two_stream_m2d/"+dataset+"/"+str(split)+"/ts_resnet101/fusion/fhn/"+fm+"/video_predict_center.pickle",'rb')
            print('use local fusion')
        else:
            fusion = open("./log_dir_with_two_stream_m2d/"+dataset+"/"+str(split)+"/ts_resnet101/fusion/non_local/"+fm+"/video_predict_center.pickle",'rb')
            print('use non-local fusion')
