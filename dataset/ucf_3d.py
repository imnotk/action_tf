...
class ucf_dataset:
    ...
    def _py_func_rgb_test_vp(rgb_path, label):
        ...
        if len(img_list) != _frame_counts:
            img_list = np.concatenate([img_list] * (_frame_counts//len(img_list)+1),axis=0)[:_frame_counts] 
...
if __name__ == '__main__':
    ...
    m = ucf_dataset (batch_size=1,frame_counts=16, eval_type='rgb',prefetch_buffer_size=1).dataset ()
    iter = m.make_one_shot_iterator ()
    for i in range(5000):
        t = time.time()
        k = iter.next()
        print (k[0].shape , k[1].shape , time.time() - t)