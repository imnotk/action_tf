...
try:
    from ts_fusion_net import Unit3d,  deep_bottleneck, _build_feature
except:
    from model.ts_fusion_net import Unit3d,  deep_bottleneck, _build_feature

try:
    from ts_non_local_fusion_helper import space_nonlocal_fusion,channel_nonlocal_fusion
except:
    from model.ts_non_local_fusion_helper import space_nonlocal_fusion,channel_nonlocal_fusion
...
class space_cross_correlation_Network(snt.AbstractModule):
    ...
    def __init__(self,num_classes=101,middle_channels=256,spatial_squeeze=True,name='FHN',use_batch_norm=False,batch_size = None,use_bias=False,fusion_mode = 'add',use_space=True):
    ...
    def _build(self, feature_pairs,is_training=False,dropout_keep_prob=1.0):
        ...
        :return:
        '''
        # we only need feature size of 28,14,7
        ...
        for i in range(feature_nums):
            if i == 0 :
                with tf.variable_scope('top_%d' % i):
                    ...
                    rgb_feature = Unit3d(output_channels=reduce_dimensions,use_batch_norm=self._use_batch_norm,activation_fn=None,use_bias=self._use_bias,name='spatial')(rgb_feature,is_training=is_training)
                    flow_feature = Unit3d(output_channels=reduce_dimensions,use_batch_norm=self._use_batch_norm,activation_fn=None,use_bias=self._use_bias,name='motion')(flow_feature,is_training=is_training)

                    spatial_attention_feature = space_nonlocal_fusion(name='rgb_flow_fusion',space=self._use_space)(rgb_feature,flow_feature,is_training)
                    motion_attention_feature = space_nonlocal_fusion(name='flow_rgb_fusion',space=self._use_space)(flow_feature,rgb_feature,is_training)
                    ...
                    mixed_feature = Unit3d(output_channels=feature_dimensions // 4,kernel_shape=(1,7,7),stride=(1,2,2),use_batch_norm = False,use_bias=self._use_bias)(mixed_feature,is_training=is_training)
                    mixed_feature = deep_bottleneck(depth=feature_dimensions,depth_bottleneck=feature_dimensions // 4,stride=(1,1,1),use_batch_norm=False,use_bias=self._use_bias,name='adaption_layer_1')(mixed_feature,is_training=is_training)
                    mixed_feature = deep_bottleneck(depth=feature_dimensions,depth_bottleneck=feature_dimensions // 4,stride=(1,1,1),use_batch_norm=False,use_bias=self._use_bias,name='adaption_layer_2')(mixed_feature,is_training=is_training)
                    mixed_feature = deep_bottleneck(depth=feature_dimensions,depth_bottleneck=feature_dimensions // 4,stride=(1,1,1),use_batch_norm=False,use_bias=self._use_bias,name='adaption_layer_3')(mixed_feature,is_training=is_training)
                ...
                with tf.variable_scope('top_%d' % i):
                    ...
                    if i != feature_nums - 1:
                        mixed_two_feature = deep_bottleneck(depth=feature_dimensions ,depth_bottleneck=feature_dimensions // 4,stride=(1,2,2),use_batch_norm=False,use_bias=self._use_bias,name='adaption_layer_1')(mixed_two_feature,is_training=is_training)
                        top_net = deep_bottleneck(depth=feature_dimensions,depth_bottleneck=feature_dimensions // 4,stride=(1,1,1),use_batch_norm=False,use_bias=self._use_bias,name='adaption_layer_2')(mixed_two_feature,is_training=is_training)
                    else:
                        mixed_two_feature = deep_bottleneck(depth=feature_dimensions,depth_bottleneck=feature_dimensions // 4,stride=(1,1,1),use_batch_norm=False,use_bias=self._use_bias,name='adaption_layer_1')(mixed_two_feature,is_training=is_training)
        ...
        for i in range(feature_nums):
            if i == 0 :
                with tf.variable_scope('top_%d' % i):
                    ...
                    spatial_attention_feature = channel_nonlocal_fusion(name='rgb_flow_fusion',space=self._use_space)(rgb_feature,flow_feature,is_training)
                    motion_attention_feature = channel_nonlocal_fusion(name='flow_rgb_fusion',space=self._use_space)(flow_feature,rgb_feature,is_training)
