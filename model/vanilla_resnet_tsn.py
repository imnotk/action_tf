...
class Resnet(snt.AbstractModule):
    ...
    def  __init__(self,num_classes = 1000,spatia_squeeze = True,unit_num = [2,2,2,2],frame_counts = 1, 
    ...
    def _build(self, inputs ,is_training ,dropout_keep_prob = 0.5):
        ...
        with tf.variable_scope(end_point):
            ...
            for i in range(num_units):
                with tf.variable_scope('unit_%d' % (i+1)):
                    if i != 0:
                        net = bottleneck(depth=64,stride=1 )(net,is_training=is_training )
                    else:
                        net = bottleneck(depth=64,stride=1 )(net,is_training=is_training )
        end_points[end_point] = net
        if self._final_endpoint == end_point: return net,end_points
        ...
        with tf.variable_scope(end_point):
            ...
            for i in range (num_units):
                with tf.variable_scope ('unit_%d' % (i + 1)):
                    if i != 0:
                        net = bottleneck (depth=128, stride=1 ) (net, is_training=is_training )
                    else:
                        net = bottleneck (depth=128, stride=2 ) (net, is_training=is_training )
        end_points[end_point] = net
        if self._final_endpoint == end_point: return net, end_points
        ...
        with tf.variable_scope(end_point):
            ...
            for i in range (num_units):
                with tf.variable_scope ('unit_%d' % (i + 1)):
                    if i != 0:
                        net = bottleneck (depth=256,stride=1 ) (net, is_training=is_training )
                    else:
                        net = bottleneck (depth=256, stride=2 ) (net, is_training=is_training )
        end_points[end_point] = net
        if self._final_endpoint == end_point: return net, end_points
        ...
        with tf.variable_scope(end_point):
            ...
            for i in range (num_units):
                with tf.variable_scope ('unit_%d' % (i + 1)):
                    if i != 0:
                        net = bottleneck (depth=512, stride=1 ) (net, is_training=is_training )
                    else:
                        net = bottleneck (depth=512, stride=2 ) (net, is_training=is_training )
        end_points[end_point] = net
        if self._final_endpoint == end_point: return net, end_points
...
if __name__ == '__main__':
    ...
    def show_mb():
        fmA = tf.placeholder(tf.float32,[None,56,56,64])
        a = tsm(fmA,frame_counts=3)
        print(a)
    ...
    #     Resnet_v1 = vanilla_Resnet(num_classes=101,final_endpoint='logits',name='resnet_v1_18',use_pool=False)
