    ...
    def _build(self, inputs ,is_training ,dropout_keep_prob = 0.5):
        ...
        with tf.variable_scope(end_point):
            ...
            for i in range(num_units):
                with tf.variable_scope('unit_%d' % (i+1)):
                    if i != 0:
                        net_no_relu = bottleneck(depth=256,depth_bottleneck=64,stride=(1,1,1))(net,is_training=is_training)
                        net = tf.nn.relu(net_no_relu)
                    else:
                        net_no_relu = bottleneck(depth=256,depth_bottleneck=64,stride=(1,1,1))(net,is_training=is_training)
                        net = tf.nn.relu(net_no_relu)
                if i % 2 == 0:
                    end_points[end_point+'_%d_no_relu' % i] = net_no_relu
        end_points[end_point] = net
        if self._final_endpoint == end_point: return net,end_points
        ...
        with tf.variable_scope(end_point):
            ...
            for i in range (num_units):
                with tf.variable_scope ('unit_%d' % (i + 1)):
                    if i != 0:
                        net_no_relu = bottleneck (depth=512, depth_bottleneck=128, stride=(1,1,1)) (net, is_training=is_training)
                        net = tf.nn.relu(net_no_relu)
                    else:
                        net_no_relu = bottleneck (depth=512, depth_bottleneck=128, stride=(1,2,2)) (net, is_training=is_training)
                        net = tf.nn.relu(net_no_relu)
                if i % 2 == 0:
                    end_points[end_point+'_%d_no_relu' % i] = net_no_relu
        end_points[end_point] = net
        ...
        with tf.variable_scope(end_point):
            ...
            for i in range (num_units):
                with tf.variable_scope ('unit_%d' % (i + 1)):
                    if i != 0:
                        net_no_relu = bottleneck (depth=1024, depth_bottleneck=256, stride=(1,1,1)) (net, is_training=is_training)
                        net = tf.nn.relu(net_no_relu)
                    else:
                        net_no_relu = bottleneck (depth=1024, depth_bottleneck=256, stride=(1,2,2)) (net, is_training=is_training)
                        net = tf.nn.relu(net_no_relu)
                if i % 2 == 0  and (i <= 6 or i >=18) :
                    end_points[end_point+'_%d_no_relu' % i] = net_no_relu
        end_points[end_point] = net
        if self._final_endpoint == end_point: return net, end_points
        ...
        with tf.variable_scope(end_point):
            ...
            for i in range (num_units):
                with tf.variable_scope ('unit_%d' % (i + 1)):
                    if i != 0:
                        net_no_relu = bottleneck (depth=2048, depth_bottleneck=512, stride=(1,1,1)) (net, is_training=is_training)
                        net = tf.nn.relu(net_no_relu)
                    else:
                        net_no_relu = bottleneck (depth=2048, depth_bottleneck=512, stride=(1,2,2)) (net, is_training=is_training)
                        net = tf.nn.relu(net_no_relu)
                if i % 2 == 0:
                    end_points[end_point+'_%d_no_relu' % i] = net_no_relu
        end_points[end_point] = net
...
if __name__ == '__main__':
    inputs = tf.placeholder(tf.float32,[None,3,224,224,3])
    with tf.variable_scope('RGB'):
        Resnet_v1 = Resnet(num_classes=101,final_endpoint='logits',name='resnet_v1_50',unit_num=[3,4,6,3])
