...
from model import net, resnet, inception_v1, inception_v2, vgg , SE, densenet , inception_v3 , inception_resnet_v2
from model import resnet_attention , bn_inception , inception_v1_non_local,vanila_resnet
import os,gc
import time
import numpy as np
import ucf_2d , hmdb_2d
import ucf_2d_val , hmdb_2d_val
...
tf.app.flags.DEFINE_string ('model_type', 'inception_v1', 'use which model vgg,inception,resnet and so on')
tf.app.flags.DEFINE_string ('preprocess_name', 'pytorch', 'preprocess type')
...
class model ():
    ...
    def init_dataset(self):
        ...
        if dataset == 'UCF101':
            self.dataset = ucf_2d.ucf_dataset (split_number=split, is_training_split=True,
                                                      batch_size=self.batch_size, epoch=1,new_length=new_length,
                                                      eval_type=eval_type,preprocess_name=preprocess_name,
    ...
    def init_type(self):
        ...
        if model_type == 'resnet50_attention':
            ...
            self._rgb_reboot_path = '/mnt/zhujian/ckpt/rgb_se_resnet50/model.ckpt'
            self._flow_reboot_path = '/mnt/zhujian/ckpt/rgb_se_resnet50/model.ckpt'
        ...
        elif model_type == 'inception_v1_non_local':
            self.base_net = 'inceptionv1_non_local'
            self._rgb_reboot_path = '/mnt/zhujian/ckpt/rgb_inception_v1/rgb_inception_v1.ckpt'
            self._flow_reboot_path = '/mnt/zhujian/ckpt/flow_inception_v1/flow_inception_v1.ckpt'
            ...
            self._rgb_reboot_path = '/mnt/zhujian/ckpt/rgb_snt_inception_v3/model.ckpt'
            self._flow_reboot_path = '/mnt/zhujian/ckpt/flow_snt_inception_v3/model.ckpt'
        elif model_type == 'inception_resnet_v2':
            self.base_net = 'inception_resnet_v2'
            self._rgb_reboot_path = "/mnt/zhujian/ckpt/rgb_snt_inception_resnet_v2/model.ckpt"
            self._flow_reboot_path = "/mnt/zhujian/ckpt/flow_snt_inception_resnet_v2/model.ckpt"

    def init_model(self):
        if eval_type in ['rgb', 'joint']:
            with tf.variable_scope ('RGB', reuse=tf.AUTO_REUSE):
                if model_type == 'resnet18':
                    self.rgb_model = vanila_resnet.Resnet (num_classes=self._NUM_CLASSES, name='resnet_v1_18')
                if model_type == 'resnet50':
                    ...
                    self.rgb_model = inception_v1_non_local.InceptionV1 (num_classes=self._NUM_CLASSES)
                    ...
                    self.rgb_model = bn_inception.BNInception (num_classes=self._NUM_CLASSES)
                elif model_type == 'inception_v3':
        ...
        if eval_type in ['flow', 'joint']:
            with tf.variable_scope ('Flow', reuse=tf.AUTO_REUSE):
                if model_type == 'resnet18':
                    self.flow_model = vanila_resnet.Resnet (num_classes=self._NUM_CLASSES, name='resnet_v1_18')
                if model_type == 'resnet50':
                    ...
                    self.flow_model = inception_v1_non_local.InceptionV1 (num_classes=self._NUM_CLASSES)
                elif model_type == 'vgg_16':
        ...
        if eval_type in ['rgb', 'joint'] :
            ...
            for var in tf.global_variables ():
                if var.name.split ('/')[0] == 'RGB':
                    ...
                    if 'logits' not in var.name and 'Logits' not in var.name and 'attention' not in var.name:
    ...
    def train(self):
        ...
        self.cross_entropy = tf.reduce_mean (-tf.reduce_sum (self.y_ * tf.log (model_predictions + 1e-10),reduction_indices=[1]))
