...
from model import TS_resnet,ts_inception_v2,ts_inception_v3 , ts_bn_inception , ts_inception_v1
from model import ts_SE
import time
import numpy as np
import os
...
class model ():
    ...
    def init_type(self):
        if model_type == 'resnet50':
            ...
            self.base_net = 'ts_se_resnet50'
            self._rgb_reboot_path = '/mnt/zhujian/ckpt/rgb_se_resnet50/model.ckpt'
            self._flow_reboot_path = '/mnt/zhujian/ckpt/flow_se_resnet50/model.ckpt'
        ...
        if eval_type in ['rgb', 'joint']:
            with tf.variable_scope ('RGB', reuse=tf.AUTO_REUSE):
                if model_type == 'resnet50':
                    ...
                    self.rgb_model = ts_SE.SE_Resnet(num_classes=self._NUM_CLASSES,final_endpoint='logits',name='resnet_v1_50')
                elif model_type == 'resnet101':
                    self.rgb_model = TS_resnet.Resnet(num_classes=self._NUM_CLASSES,final_endpoint='logits',name='resnet_v1_101',unit_num=[3, 4, 23, 3])
                elif model_type == 'resnet152':
                    self.rgb_model = TS_resnet.Resnet(num_classes=self._NUM_CLASSES,final_endpoint='logits',name='resnet_v1_152',unit_num=[3, 8, 36, 3])
                elif model_type == 'inception_v1':
                    self.rgb_model = ts_inception_v1.InceptionV1(num_classes=self._NUM_CLASSES)
                elif model_type == 'inception_v2':
                    self.rgb_model = ts_inception_v2.InceptionV2(num_classes=self._NUM_CLASSES)
                elif model_type == 'inception_v3':
                    self.rgb_model = ts_inception_v3.InceptionV3(num_classes=self._NUM_CLASSES)
        ...
        if eval_type in ['flow', 'joint']:
            with tf.variable_scope ('Flow', reuse=tf.AUTO_REUSE):
                if model_type == 'resnet50':
                    ...
                    self.flow_model = ts_SE.SE_Resnet(num_classes=self._NUM_CLASSES,final_endpoint='logits',name='resnet_v1_50')
                elif model_type == 'resnet101':
                    ...
                    self.flow_model = TS_resnet.Resnet(num_classes=self._NUM_CLASSES,final_endpoint='logits',name='resnet_v1_152',unit_num=[3, 8, 36, 3])
                elif model_type == 'inception_v1':
                    self.flow_model = ts_inception_v1.InceptionV1(num_classes=self._NUM_CLASSES)
                elif model_type == 'inception_v2':
                    self.flow_model = ts_inception_v2.InceptionV2(num_classes=self._NUM_CLASSES)
                elif model_type == 'inception_v3':
                    self.flow_model = ts_inception_v3.InceptionV3(num_classes=self._NUM_CLASSES)
